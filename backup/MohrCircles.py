...
def hoek_brown(sigma3,A,B):
    return sigma3 + (A*sigma3 + B**2)**0.5
...
class MohrCircles(QtGui.QWidget):
    def __init__(self):
        ...
        self.setupGUI()
        self.addEnvelopeButton.clicked.connect(self.addEnvelope)
    def setupGUI(self):
    ...
    def start(self):
        ...
        self.addEnvelope()
    def addEnvelope(self,type='Mohr Coulomb'):
        ...
        frictionItem = pg.TreeWidgetItem(['Friction Angle'])
        cohesionItem = pg.TreeWidgetItem(['Cohesion'])
        item.addChild(colorItem)
        item.addChild(frictionItem)
        item.addChild(cohesionItem)
        frictionBox = pg.SpinBox(value=50, step=1)
        cohesionBox = pg.SpinBox(value=1e10, step=10)
        ...
        frictionItem.setWidget(2,frictionBox)
        cohesionItem.setWidget(2,cohesionBox)
    ...
    def getEnvelope(self,eName=None):
        ...
        if (s1!=[]) and (s3!=[]):
            angle,cohesion = self.computeEnvelope(s1,s3)
            self.fBoxes[eName].setValue(angle)
            self.cBoxes[eName].setValue(cohesion)
    def computeEnvelope(self,s1,s3):
        s1 = np.array(s1)
        s3 = np.array(s3)
        A = np.array([s3, np.ones(len(s3))]).T
        a,b = np.linalg.lstsq(A,s1)[0]
    ...
    def plot(self):
        ...
        for eName in self.eNames:
            ...
            pen = pg.mkPen(color=color, width=2)
            angle = self.fBoxes[eName].value()
            a = np.tan(np.radians(angle))
            b = self.cBoxes[eName].value()
            y = a*self.env_x + b
            self.plt.plot(self.env_x,y,pen=pen)
